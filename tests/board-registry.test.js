...
const cli = resolve(process.cwd(), 'bin/switchman.js');
...
test('board start creates a worktree and registers the session', () => {
  const root = mkdtempSync(join(tmpdir(), 'switchman-board-'));
  ...
  try {
    ...
    execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          SWITCHMAN_BOARD_ROOTS_FILE: process.env.SWITCHMAN_BOARD_ROOTS_FILE,
        },
      },
    );
...
test('registry lock blocks concurrent board writes', () => {
  const root = mkdtempSync(join(tmpdir(), 'switchman-board-lock-'));
  const repo = join(root, 'repo');
  const registry = join(root, 'sessions.json');
  const worktreesDir = join(root, 'worktrees');
